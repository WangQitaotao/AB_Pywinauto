...
from common.all_paths import BASE_DIR, LOGS_PATH

# 配置日志文件名称及路径
log_path = os.path.join(BASE_DIR, 'logs')     # log_path为存放日志的路径
if not os.path.exists(LOGS_PATH):
    os.mkdir(LOGS_PATH)    # 若不存在logs文件夹，则自动创建
...
# 装饰器，删除多余的日志文件（始终保持日志文件只有6个）
def call_on_delete_logs(func):
...
def delete_logs():
    try:
        folder_num = os.listdir(LOGS_PATH)
        folder_limit = 6
        if len(folder_num) > folder_limit:
            delete_count = len(folder_num) - folder_limit  # 计算需要删除的文件夹数量
            folders_to_delete = folder_num[:delete_count]  # 选择要删除的文件夹
            for folder in folders_to_delete:
                folders_to_delete_path = os.path.join(LOGS_PATH, folder)
                os.remove(folders_to_delete_path)
    except Exception:
        raise
